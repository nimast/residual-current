...
void setup() {
    ...
    // The E-Paper display is 1600×1200, but the buffer format might need extra space
    // Buffer is storing 8 pixels per byte (1 bit per pixel)
    // Each 2 pixels need 1 byte for encoding which color it is
    // See the Waveshare documentation for details on the format
    // Using a conservative estimate
    
    // Calculate buffer size (roughly 1/2 of total pixels)
